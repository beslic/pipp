...
public class ChartData {
    ...
    public static List<String> getData(Anketa a) {
        ...
        Map<Odgovor, Long> brojOdgovora = new HashMap<>();
        a.getIspunjavanja().forEach(i -> i.getOdgovori().values().stream()
                .forEach(v -> brojOdgovora.compute(v, (key, val) -> (val == null) ? 1 : val + 1)));
        a.getPitanja().stream().forEach(p -> data.add(createData(p, brojOdgovora)));
        return data;
    }

    private static String createData(Pitanje p, Map<Odgovor, Long> brojOdgovora) {
        JsonArray data = new JsonArray();
        data.add(getDataSeries(getDataPoints(brojOdgovora, p), p.getTextPitanje()));
        Gson gson = new Gson();
